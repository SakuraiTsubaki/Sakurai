...
def gba_checksum(data): return (-sum(data[0xA0:0xBD]) - 0x19) & 0xFF

def inspect(path):
    data=path.read_bytes(); code=data[0xAC:0xB0].decode("ascii","replace"); ver=data[0xBC]
...
if __name__=="__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("rom",nargs="+"); ns=ap.parse_args(); print(json.dumps([inspect(pathlib.Path(x)) for x in ns.rom],indent=2))
