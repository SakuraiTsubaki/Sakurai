...
Stock `SpeciesInfo` is 28 bytes and includes six `u8` base stats, two `u8` types, `u8` catch rate, `u8` base EXP yield, EV-yield bitfields, two `u16` held items, two `u8` egg groups and two `u8` ability IDs.

Required changes:

- **Ability ID:** `u8` is a 255-ID ceiling. Move to `u16` in the expanded runtime/table format.
...
Stock layout is `gEvolutionTable[NUM_SPECIES][5]`.
...
Replace this with a pointer/index table to variable-length evolution records terminated by an END record.
