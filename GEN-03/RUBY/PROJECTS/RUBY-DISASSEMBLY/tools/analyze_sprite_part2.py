#!/usr/bin/env python3
"""Verify/fingerprint Pokémon Ruby sprite.c Part 2.

Scope: AnimCmd_frame through InitSpriteAffineAnim.  The function-size layout is
common to every verified Ruby target; only the Part-2 base address and code
fingerprints vary. ROMs are read-only inputs and are never emitted.
"""
...
def main() -> int:
    ...
    p.add_argument("--layout", type=Path, default=Path("symbols/sprite_part2_layout.csv"))
    p.add_argument("--targets", type=Path, default=Path("symbols/sprite_part2_targets.csv"))
    p.add_argument("--out", type=Path, default=Path("symbols/sprite_part2_fingerprints.csv"))
    ...
    for target in targets:
        ...
        base = int(target["part2_base_offset"], 16)
        ...
        if len(region) != expected_size or sha1(region) != expected_sha1:
            raise SystemExit(f"{target_id}: Part-2 region verification failed")

        for symbol, rel, size in layout:
            start = base + rel
    ...
    if args.verify_only:
        print(f"verified {len(targets)} targets / {len(rows)} function records")
        return 0

    args.out.parent.mkdir(parents=True, exist_ok=True)
    with args.out.open("w", encoding="utf-8", newline="") as fp:
        fieldnames = ["target", "symbol", "file_offset", "address", "thumb_address", "size", "sha1"]
