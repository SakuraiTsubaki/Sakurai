...
mkdir -p "$TMP" "$VENDOR_ROOT"/{bin,source,licenses,eden,ghidra,dotnet,ryubing,aarch64,llvm,buildtools}
...
source_archive() {
  ...
  git -C "$TMP/$name" archive --format=tar.gz --output="$VENDOR_ROOT/source/${name}-${commit}.tar.gz" "$commit"
}

cat > "$VENDOR_ROOT/bin/_materialize" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
if [[ ! -f "$STAMP" ]] || [[ "$(cat "$STAMP" 2>/dev/null || true)" != "$EXPECTED" ]]; then
  ...
  cat "$DIR/$STEM.part."* | zstd -d -q | tar -xf - -C "$CACHE"
  printf '%s\n' "$EXPECTED" > "$STAMP"
fi
printf '%s\n' "$CACHE"
EOF
chmod 0755 "$VENDOR_ROOT/bin/_materialize"

# hactool: executable + exact source snapshot.
...
cp "$TMP/hactool/LICENSE" "$VENDOR_ROOT/licenses/hactool-LICENSE" 2>/dev/null || true

# Eden: store the emulator itself in Git as <=45 MiB chunks.
...
import json,sys
x=json.load(open(sys.argv[1],encoding='utf-8')); vals=[]; stack=[x]
while stack:
    v=stack.pop()
...
split -b "$CHUNK_SIZE" -d -a 3 "$TMP/Eden.AppImage" "$VENDOR_ROOT/eden/Eden.AppImage.part."
...
# Ghidra + JDK 21 + current Switch Loader extension, bundled ready to execute.
...
READY="$TMP/ghidra-ready"; mkdir -p "$READY"; cp -a "$GHIDRA_HOME" "$READY/ghidra"; cp -a "$(dirname "$(dirname "$(readlink -f "$(command -v javac)")")")" "$READY/jdk"
tar -C "$READY" -cf - ghidra jdk | zstd -19 -T0 -q -o "$TMP/ghidra-ready.tar.zst"
split_store "$TMP/ghidra-ready.tar.zst" "$VENDOR_ROOT/ghidra" ghidra-ready.tar.zst
cat > "$VENDOR_ROOT/bin/ghidra" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
B="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; V="$B/../ghidra"; C="${XDG_CACHE_HOME:-/tmp}/sakurai-ghidra-12.1.3"; "$B/_materialize" "$V" ghidra-ready.tar.zst "$C" >/dev/null
export JAVA_HOME="$C/jdk"; exec "$C/ghidra/ghidraRun" "$@"
EOF
cat > "$VENDOR_ROOT/bin/ghidra-headless" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
B="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; V="$B/../ghidra"; C="${XDG_CACHE_HOME:-/tmp}/sakurai-ghidra-12.1.3"; "$B/_materialize" "$V" ghidra-ready.tar.zst "$C" >/dev/null
export JAVA_HOME="$C/jdk"; exec "$C/ghidra/support/analyzeHeadless" "$@"
EOF
chmod 0755 "$VENDOR_ROOT/bin/ghidra" "$VENDOR_ROOT/bin/ghidra-headless"

# .NET 10 SDK plus a prebuilt Ryubing runtime.
...
git -C "$TMP/Ryubing" archive --format=tar.gz --output="$VENDOR_ROOT/source/Ryubing-${RYUBING_COMMIT}.tar.gz" "$RYUBING_COMMIT"
READY="$TMP/dotnet-ready"; mkdir -p "$READY"; cp -a "$TMP/dotnet" "$READY/dotnet"; cp -a "$TMP/ryubing-build" "$READY/ryubing"
tar -C "$READY" -cf - dotnet ryubing | zstd -19 -T0 -q -o "$TMP/dotnet-ryubing.tar.zst"
split_store "$TMP/dotnet-ryubing.tar.zst" "$VENDOR_ROOT/dotnet" dotnet-ryubing.tar.zst
cat > "$VENDOR_ROOT/bin/dotnet" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
B="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; C="${XDG_CACHE_HOME:-/tmp}/sakurai-dotnet-ryubing"; "$B/_materialize" "$B/../dotnet" dotnet-ryubing.tar.zst "$C" >/dev/null; exec "$C/dotnet/dotnet" "$@"
EOF
cat > "$VENDOR_ROOT/bin/ryubing" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
B="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; C="${XDG_CACHE_HOME:-/tmp}/sakurai-dotnet-ryubing"; "$B/_materialize" "$B/../dotnet" dotnet-ryubing.tar.zst "$C" >/dev/null
export DOTNET_ROOT="$C/dotnet"; export PATH="$DOTNET_ROOT:$PATH"
if [[ -x "$C/ryubing/Ryujinx" ]]; then exec "$C/ryubing/Ryujinx" "$@"; fi
DLL="$(find "$C/ryubing" -maxdepth 1 -iname 'Ryujinx*.dll' | head -n1)"; exec "$C/dotnet/dotnet" "$DLL" "$@"
EOF
chmod 0755 "$VENDOR_ROOT/bin/dotnet" "$VENDOR_ROOT/bin/ryubing"

# pkNX pinned source snapshot for offline format/data reference.
source_archive https://github.com/kwsch/pkNX.git "$PKNX_COMMIT" pkNX

# Portable command-line AArch64 binutils with their host shared-library dependencies.
...
tar -C "$READY" -cf - bin lib | zstd -19 -T0 -q -o "$TMP/aarch64-tools.tar.zst"; split_store "$TMP/aarch64-tools.tar.zst" "$VENDOR_ROOT/aarch64" aarch64-tools.tar.zst
for n in objdump readelf nm objcopy; do cat > "$VENDOR_ROOT/bin/aarch64-linux-gnu-$n" <<EOF
#!/usr/bin/env bash
set -euo pipefail
B="\$(cd "\$(dirname "\${BASH_SOURCE[0]}")" && pwd)"; C="\${XDG_CACHE_HOME:-/tmp}/sakurai-aarch64"; "\$B/_materialize" "\$B/../aarch64" aarch64-tools.tar.zst "\$C" >/dev/null; export LD_LIBRARY_PATH="\$C/lib:\${LD_LIBRARY_PATH:-}"; exec "\$C/bin/aarch64-linux-gnu-$n" "\$@"
EOF
chmod 0755 "$VENDOR_ROOT/bin/aarch64-linux-gnu-$n"; done

# LLVM/Clang/lld tree for reconstruction builds and AArch64 disassembly.
LLVM_ROOT="$(llvm-config --prefix)"; tar -C "$(dirname "$LLVM_ROOT")" -cf - "$(basename "$LLVM_ROOT")" | zstd -19 -T0 -q -o "$TMP/llvm.tar.zst"; split_store "$TMP/llvm.tar.zst" "$VENDOR_ROOT/llvm" llvm.tar.zst
for n in clang clang++ llvm-objdump llvm-readelf llvm-nm llvm-objcopy ld.lld; do cat > "$VENDOR_ROOT/bin/$n" <<EOF
#!/usr/bin/env bash
set -euo pipefail
B="\$(cd "\$(dirname "\${BASH_SOURCE[0]}")" && pwd)"; C="\${XDG_CACHE_HOME:-/tmp}/sakurai-llvm"; "\$B/_materialize" "\$B/../llvm" llvm.tar.zst "\$C" >/dev/null; X="\$(find "\$C" -type f -path '*/bin/$n' | head -n1)"; exec "\$X" "\$@"
EOF
chmod 0755 "$VENDOR_ROOT/bin/$n"; done

# Small build helpers are stored directly.
cp "$(command -v ninja)" "$VENDOR_ROOT/bin/ninja"
...
cat > "$VENDOR_ROOT/VERSIONS.md" <<EOF
# Generation IX vendored Switch toolchain
- hactool: $HACTOOL_COMMIT
- Ghidra: $GHIDRA_VERSION + Ghidra-Switch-Loader $SWITCH_LOADER_COMMIT + JDK 21
- .NET SDK: $DOTNET_VERSION
- pkNX source: $PKNX_COMMIT
- Ryubing: $RYUBING_COMMIT
- Eden: release recorded in eden/release.json; AppImage bytes are committed as chunks
