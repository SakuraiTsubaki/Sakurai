...
atlas=list(csv.DictReader(open(root/'data/semantic/bank_atlas.csv',encoding='utf-8')))
...
rels=list(csv.DictReader(open(root/'data/semantic/relationship_bank_diffs.csv',encoding='utf-8')))
if len(rels)!=512: errors.append(f'relationship bank rows {len(rels)} != 512')
if errors:
 print('FAIL'); [print('-',e) for e in errors]; sys.exit(1)
print('PASS semantic layer:',len(rows),'banks,',len(atlas),'bank instances,',len(sec),'sections,',len(rels),'relationship-bank rows')
